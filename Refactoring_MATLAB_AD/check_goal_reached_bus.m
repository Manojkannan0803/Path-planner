function [should_stop, stop_reason, goal_errors] = check_goal_reached_bus(input_bus)
    ...
    %     .vehicle_config  - VehicleTypeBus (type, name, has_articulation)
    %
    % OUTPUTS:
    ...
    %
    % SEE ALSO: check_goal_reached, create_vehicle_state, create_tolerance_config, create_vehicle_config
    ...
    % Get vehicle type from name field (convert from uint8 array)
    if isfield(vehicle_config, 'name')
        % Convert uint8 array to string and remove padding zeros
        name_uint8 = vehicle_config.name;
        name_uint8 = name_uint8(name_uint8 ~= 0); % Remove padding zeros
        vehicle_type = char(name_uint8);
        
        % Validate vehicle type
        if isempty(vehicle_type)
    ...
    % Determine if vehicle has articulation based on type
