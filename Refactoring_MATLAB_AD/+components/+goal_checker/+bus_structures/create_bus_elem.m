function bus_elem = create_bus_elem(name, dim, type, varargin)
    ...
    % Set Units property if supported (newer Simulink versions)
    if ~isempty(units)
        if components.goal_checker.bus_structures.check_simulink_property_support('Units')
            bus_elem.Units = units;
        else
            % Units property not supported in this Simulink version
            % Add to description instead
            if isempty(bus_elem.Description)
                bus_elem.Description = sprintf('Units: %s', units);
            else
                bus_elem.Description = sprintf('%s (Units: %s)', bus_elem.Description, units);
            end
        end
    end
