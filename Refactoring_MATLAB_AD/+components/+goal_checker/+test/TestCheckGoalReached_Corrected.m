classdef TestCheckGoalReached_Corrected < matlab.unittest.TestCase
    ...
    methods (Test)
        ...
        function testValidInputs_ArticulatedVehicle(testCase)
            ...
            required_fields = {'position_error', 'orientation_error', 'articulation_error', ...
                'position_ok', 'orientation_ok', 'articulation_ok'};
        ...
        function testBoundaryValues_OrientationTolerance(testCase)
            ...
            for i = 1:length(test_cases)
                ...
                input_bus.current_state = current_state;
                input_bus.goal_state = goal_state;
                input_bus.tolerances = struct('position_tolerance', 100, 'orientation_tolerance', tolerance, 'articulation_tolerance', 180);
        ...
        function testDecisionTable_ToleranceCombinations(testCase)
            ...
            for i = 1:length(test_cases)
                ...
                input_bus.current_state = current_state;
                input_bus.goal_state = goal_state;
                input_bus.tolerances = struct('position_tolerance', tc.tolerances(1), 'orientation_tolerance', tc.tolerances(2), 'articulation_tolerance', tc.tolerances(3));
        ...
        function testStateTransition_ApproachingGoal(testCase)
            ...
            for step = 1:steps
                ...
                input_bus.current_state = current_state;
                input_bus.goal_state = goal_state;
                input_bus.tolerances = struct('position_tolerance', tolerances(1), 'orientation_tolerance', tolerances(2), 'articulation_tolerance', tolerances(3));
        ...
        function testErrorConditions_InvalidInputs(testCase)
            ...
            valid_vehicle_config.name = uint8([double(vehicle_name), zeros(1, 20-length(vehicle_name))]);
            
            % Test invalid current_state
            testCase.verifyError(@() check_goal_reached_bus(struct('current_state', 'invalid', 'goal_state', valid_state, 'tolerances', struct('position_tolerance', 2, 'orientation_tolerance', 15, 'articulation_tolerance', 10), 'vehicle_config', valid_vehicle_config)), ...
                'MATLAB:InputParser:ArgumentFailedValidation');
            
            % Test invalid goal_state
            testCase.verifyError(@() check_goal_reached_bus(struct('current_state', valid_state, 'goal_state', 'invalid', 'tolerances', struct('position_tolerance', 2, 'orientation_tolerance', 15, 'articulation_tolerance', 10), 'vehicle_config', valid_vehicle_config)), ...
                'MATLAB:InputParser:ArgumentFailedValidation');
            
            % Test invalid vehicle type (empty name)
            invalid_vehicle_config = struct();
            invalid_vehicle_config.name = uint8(zeros(1, 20));  % All zeros (empty name)
            testCase.verifyError(@() check_goal_reached_bus(struct('current_state', valid_state, 'goal_state', valid_state, 'tolerances', struct('position_tolerance', 2, 'orientation_tolerance', 15, 'articulation_tolerance', 10), 'vehicle_config', invalid_vehicle_config)), ...
                'MATLAB:error');
        end
        ...
        function testErrorConditions_MissingFields(testCase)
            ...
            for i = 1:length(incomplete_states)
                ...
                input_bus.goal_state = goal_state;
                input_bus.tolerances = struct('position_tolerance', 2, 'orientation_tolerance', 15, 'articulation_tolerance', 10);
        ...
        function testEdgeCases_ExtremeValues(testCase)
            ...
            input_bus1.goal_state = goal_state;
            input_bus1.tolerances = struct('position_tolerance', 1e7, 'orientation_tolerance', 15, 'articulation_tolerance', 10);
            ...
            input_bus2.tolerances = struct('position_tolerance', 1e-6, 'orientation_tolerance', 1e-3, 'articulation_tolerance', 1e-3);
        ...
        function testParameterized_VehicleTypes(testCase, vehicle_types)
            ...
            input_bus.vehicle_config = struct();
            input_bus.vehicle_config.name = uint8([double(vehicle_types), zeros(1, 20-length(vehicle_types))]);
            
            [should_stop, stop_reason, goal_errors] = check_goal_reached_bus(input_bus);
            
            % Verify basic functionality for each vehicle type
            testCase.verifyClass(should_stop, 'logical');
            testCase.verifyEqual(goal_errors.debug_info.vehicle_type, vehicle_types);
        ...
        function testParameterized_ToleranceScenarios(testCase, tolerance_scenarios)
            ...
            input_bus.current_state = current_state;
            input_bus.goal_state = goal_state;
            input_bus.tolerances = struct('position_tolerance', tol(1), 'orientation_tolerance', tol(2), 'articulation_tolerance', tol(3));
            ...
            testCase.verifyEqual(errors.debug_info.position_tolerance, tol(1));
            testCase.verifyEqual(errors.debug_info.orientation_tolerance, tol(2));
            testCase.verifyEqual(errors.debug_info.articulation_tolerance, tol(3));
        ...
        function testNumericalStability_AngleNormalization(testCase)
            ...
            for i = 1:length(test_angles)
                ...
                input_bus.current_state = current_state;
                input_bus.goal_state = goal_state;
                input_bus.tolerances = struct('position_tolerance', 100, 'orientation_tolerance', 180, 'articulation_tolerance', 180);
        ...
        function testIntegration_DefaultParameters(testCase)
            ...
            testCase.verifyEqual(goal_errors.debug_info.position_tolerance, 2.0);
            testCase.verifyEqual(goal_errors.debug_info.orientation_tolerance, 15.0);
            testCase.verifyEqual(goal_errors.debug_info.articulation_tolerance, 10.0);
            testCase.verifyEqual(goal_errors.debug_info.vehicle_type, 'articulated');
        end
    end
