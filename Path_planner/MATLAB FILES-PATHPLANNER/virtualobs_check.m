...
function [A1,A2] = virtualobs_check(cur,obsc,obsx,obsy)
...
%         obsxtemp=obsxtemp(obsxtemp~=0);
%         obsytemp=obsytemp(obsytemp~=0);
%         in1 = InPolygon(xv_obs,yv_obs,obsxtemp,obsytemp); % check whether cur is hitting obs
% %     end
%     if in1
%        A1 = cur.pred.tf:9:cur.pred.tf+90; % Change the discretization interval as per user-defined cases
%        A2 = cur.pred.tf-90:9:cur.pred.tf; 
%     else
%        A1 = cur.pred.tf:9:cur.pred.tf+45;
%        A2 = cur.pred.tf-45:9:cur.pred.tf;
%     end
