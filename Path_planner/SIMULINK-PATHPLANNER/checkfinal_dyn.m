function checkfin = checkfinal_dyn(xp,yp,thetap,gammap,xp_MO,yp_MO,thetap_MO,obsc,obsx,obsy)
coder.extrinsic('InPolygon');
conddyn = false(1,1);
condobssta = false(1,1);
...
conddyn = any(in2);
    if conddyn == 1
        checkfin = 1; % intersect
    else
        checkfin = 0; % doesn't intersect
    end
