function hc_d = rectheurslx(ip,f_statex,f_statey)
...
c = ip.xa<28 && ip.ya>54;
if a
    length = (abs(ip.xa-xlim1)+abs(ip.ya-ylim2)+abs(xlim1-f_statex)+abs(ylim2-f_statey))*3;%*3
elseif b
    length = (abs(ip.ya-ylim1)+abs(ip.xa-xlim1)+abs(ylim1-ylim2)+abs(xlim1-f_statex)+abs(ylim2-f_statey))*3;%*3
elseif c
    length = (abs(ip.ya-ylim2)+abs(ylim2-f_statey)+abs(ip.xa-f_statex))*2.8;%*2.8
elseif in5
    length = 0;
else
